...
func (r *fakeUserRepo) FindByEmail(ctx context.Context, email string) (*domain.User, error)  { return r.byEmail[email], nil }
func (r *fakeUserRepo) FindByID(ctx context.Context, id string) (*domain.User, error)        { return r.byID[id], nil }
...
func (r *fakeUserRepo) AssignPersonsToUser(ctx context.Context, u string, p []string) error  { return nil }
func (r *fakeUserRepo) GetAssignedPersonIDs(ctx context.Context, u string) ([]string, error) { return nil, nil }
...
func (s *fakeAuthSession) StoreSession(ctx context.Context, userID, tokenID string, exp time.Duration) error { return nil }
func (s *fakeAuthSession) IsSessionBlacklisted(ctx context.Context, tokenID string) (bool, error)            { return false, nil }
func (s *fakeAuthSession) InvalidateSession(ctx context.Context, tokenID string, exp time.Duration) error     { return nil }
...
func TestLoginReturnsAccessAndRefreshToken(t *testing.T) {
	...
	access, refresh, user, err := svc.Login(context.Background(), "a@b.com", "correct-password")
	require.NoError(t, err)
	assert.NotEmpty(t, access)
	assert.NotEmpty(t, refresh)
	assert.Equal(t, "user-1", user.ID)

	claims, err := security.ValidateToken(access, testJWTSecret)
	require.NoError(t, err)
...
func TestRefreshRotatesRefreshTokenAndReturnsNewAccess(t *testing.T) {
	svc, repo, _ := newTestAuthService()
	mustCreateUser(t, repo, "user-1", "a@b.com", "correct-password")
	_, initialRefresh, _, err := svc.Login(context.Background(), "a@b.com", "correct-password")
	require.NoError(t, err)

	access, newRefresh, user, err := svc.Refresh(context.Background(), initialRefresh)
	require.NoError(t, err)
	assert.NotEmpty(t, access)
	assert.NotEmpty(t, newRefresh)
	assert.Equal(t, "user-1", user.ID)
	assert.NotEqual(t, initialRefresh, newRefresh)
}

func TestLogoutEverywhereRevokesAllSessions(t *testing.T) {
	svc, repo, sessions := newTestAuthService()
	mustCreateUser(t, repo, "user-1", "a@b.com", "correct-password")
	_, _, _, err := svc.Login(context.Background(), "a@b.com", "correct-password")
	require.NoError(t, err)
...
func TestLogoutRevokesCurrentRefreshSession(t *testing.T) {
	svc, repo, sessions := newTestAuthService()
	mustCreateUser(t, repo, "user-1", "a@b.com", "correct-password")
	_, refresh, _, err := svc.Login(context.Background(), "a@b.com", "correct-password")
	require.NoError(t, err)

	require.NoError(t, svc.Logout(context.Background(), "token-id", refresh))
	assert.True(t, sessions.revokedTok[refresh])
}